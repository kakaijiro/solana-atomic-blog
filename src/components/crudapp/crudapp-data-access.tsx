...
export function useCrudappProgramAccount({ account }: { account: PublicKey }) {
  ...
  const updateEntry = useMutation<string, Error, CreateEntryArgs>({
    ...
    mutationFn: async ({ title, message }) => {
      return program.methods.updateJouralEntry(title, message).rpc();
    },
    onSuccess: (signature) => {
      transactionToast(signature);
      accounts.refetch();
    ...
    onError: (error) => {
      toast.error(`Error updateing entry: ${error.message}`);
    },
  });
  ...
  //   mutationFn: () =>
  //     program.methods.close().accounts({ crudapp: account }).rpc(),
  //   onSuccess: (tx) => {
  //     transactionToast(tx);
